...
public class ConvenientBanner<T> extends LinearLayout {
    ...
    public ConvenientBanner setPages(CBViewHolderCreator holderCreator, List<T> datas) {
        ...
        viewPager.setOnTouchListener(myOnTouchListener);
        return this;
    }
    ...
    public ConvenientBanner setPageIndicator(int[] page_indicatorId) {
        ...
        if (mDatas != null && !mDatas.isEmpty() && loPageTurningTitle != null) {
            StoryData bannerData = (StoryData) mDatas.get(0);
            if (bannerData != null && !TextUtils.isEmpty(bannerData.getTitle())) {
                loPageTurningTitle.setText(bannerData.getTitle());
            }
        }
    ...
    private OnCBLoopViewPaperTouchListner onCBLoopViewPaperTouchListner;

    public interface OnCBLoopViewPaperTouchListner {
        void onParentSwipeRefreshEnabled(Boolean enabled);
    }

    public void setOnCBLoopViewPaperTouchListner(OnCBLoopViewPaperTouchListner listner) {
        this.onCBLoopViewPaperTouchListner = listner;
    }
        ...
        public boolean onTouch(View v, MotionEvent event) {
            switch (event.getAction()) {
                case MotionEvent.ACTION_MOVE:
                    if (onCBLoopViewPaperTouchListner != null) {
                        onCBLoopViewPaperTouchListner.onParentSwipeRefreshEnabled(false);
                    }
                    break;
                case MotionEvent.ACTION_UP:
                case MotionEvent.ACTION_CANCEL:
                    if (onCBLoopViewPaperTouchListner != null) {
                        onCBLoopViewPaperTouchListner.onParentSwipeRefreshEnabled(true);
                    }
                    break;
            }
            return false;
